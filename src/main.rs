...
async fn main() -> Result<()> {
    ...
    if let Some(port) = args.serial_port {
    ...
    } else if config.meshtastic.serial_port.is_none() {
        // If no serial port specified, try auto-detection
        match serial_detector::detect_meshtastic_port().await {
            ...
            Err(e) => {
                return Err(anyhow::anyhow!("Failed to auto-detect serial port: {}. Please specify with --serial-port", e));
            }
        }
    }
